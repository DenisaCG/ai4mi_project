...
course release, 1-4 once the aorta annotation is present).
...
# All labels this dataset could ever have; main() narrows to the ones that
# actually have voxels before drawing.
ALL_LABELS = list(LABEL_COLORS)
...
def main():
    ...
    # Labels with at least one piece somewhere in this dataset -- not hardcoded,
    # so the same figure works whether the aorta annotation (label 4) is present.
    present_labels = sorted(int(v) for v in rows_df.label.unique())
    shares = piece_shares(rows_df)

    sns.set_theme(style="white", font_scale=1.1)
    fig, axes = plt.subplots(1, len(present_labels), figsize=(5 * len(present_labels), 5.2), sharey=True)
    for ax, k in zip(axes, present_labels):
    ...
    out = out_subdir(args.profile_dir, "connected_components") / "connected_components.png"
