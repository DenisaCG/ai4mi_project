...
Left: patient 01's scan as eight slices stacked in 3D, each pixel coloured
...
Usage:
    python dataset_analysis/profile_figures/f04_scan_intensity/centre_edge_outside.py --profile-dir figures/profile
"""
...
def draw_patient_stack(fig, ax, plot_regions: np.ndarray, plot_indices: np.ndarray, spacing: np.ndarray) -> None:
    """Draw patient 01's eight slices in 3D, coloured by region, with a region legend.
...
def draw_region_panels(fig, gs, per_patient_pct: dict, pooled_pct: dict, norm: dict) -> None:
    ...
    Args:
        ...
        norm: nnU-Net normalisation stats from nnunet_ct_normalisation(), for the clip lines.
    """
...
def main():
    """Generate the centre/edge/outside scan intensity figure."""
    ...
    out_path = output_path(args.profile_dir, "centre_edge_outside.png")
