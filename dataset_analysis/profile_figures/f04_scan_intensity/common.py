...
figures/profile/04_scan_intensity/.
"""
...
def nnunet_ct_normalisation(hists: dict) -> dict:
    """nnU-Net's CT normalisation values, recomputed from its own foreground sample.

    nnU-Net pools a random sample of labelled voxels from every training case,
...
def normalise(hu: np.ndarray, norm: dict) -> np.ndarray:
    """Apply nnU-Net's CT normalisation (clip, subtract mean, divide by std)."""
...
def clip_lines(ax, norm: dict, label: bool = False) -> None:
    """Dashed vertical lines at nnU-Net's clip values, optionally labelled above the axes."""
...
def draw_slice_stack(ax, colors: list[np.ndarray], spacing: np.ndarray, indices: np.ndarray, step: int = 4) -> None:
    """Draw axial slices as flat coloured planes stacked at their height in a 3D axis.
