#!/usr/bin/env python3
"""Scan intensity before and after nnU-Net's CT normalisation.

Top row: patient 01's CT as two 3D slice stacks, one colored by raw HU and one
by the value after normalise() -- clip to nnU-Net's foreground-sample
percentiles, then subtract the mean and divide by the standard deviation
(nnunet_ct_normalisation()). Opacity comes from see_through_alpha() on the raw HU:
...
through normalise(). range_percent() folds voxels outside each panel's range
into its first/last bin, so the right panel's end bins are exactly the mass
normalise() itself clips.

Usage:
    python dataset_analysis/profile_figures/f04_scan_intensity/nnunet_normalisation.py --profile-dir figures/profile
"""
...
def stack_rgba(ct: np.ndarray, values: np.ndarray, indices: np.ndarray, cmap, norm: Normalize) -> list[np.ndarray]:
    ...
    Args:
        ...
        values: Quantity to color by, same shape as ct -- raw HU or normalised.
...
def top_row(fig, gs, data_dir: Path, norm: dict) -> None:
    """Draw the example patient's raw and normalised CT as two 3D slice stacks.
    ...
    Args:
        ...
        norm: nnU-Net normalisation stats from nnunet_ct_normalisation().
    """
    ...
    normalised = normalise(ct.astype(np.float32), norm)
    display_lo, display_hi = normalise(np.array([norm["clip_low"], norm["clip_high"]]), norm)
    ax_norm = fig.add_subplot(gs[1], projection="3d")
    norm_norm = Normalize(vmin=display_lo, vmax=display_hi, clip=True)
    draw_slice_stack(ax_norm, stack_rgba(ct, normalised, indices, cmap, norm_norm), spacing, indices)
    ax_norm.set_title(f"{patient_name(EXAMPLE_PATIENT)}, after nnU-Net normalisation", fontsize=13, loc="left")
    add_stack_colorbar(fig, ax_norm, cmap, norm_norm, "value after nnU-Net normalisation")
...
def norm_bin_percents(hists: dict, patients: list[str], norm: dict) -> tuple[np.ndarray, list[np.ndarray]]:
    """Percentage of each patient's scan voxels in fixed-width HU ranges after normalisation.

    Bin edges run from clip_low to clip_high in HU and are then mapped through
    normalise(). Voxels outside the clip range pile into the first/last range,
    exactly matching the clipping normalise() itself applies.
    ...
    Returns:
        Tuple of (bin edges in normalised units, list of per-patient bin heights in percent).
    """
    ...
    return normalise(edges_hu, norm), heights
...
def bottom_row(fig, gs, hists: dict, patients: list[str], norm: dict) -> None:
    """Draw the two whole-scan histogram panels (raw and normalised), all patients.
    ...
    ax_norm.set_xlabel("value after nnU-Net normalisation")
    ax_norm.set_ylabel("% of the scan's voxels")
    ax_norm.set_xlim(edges_norm[0] - 0.15, edges_norm[-1] + 0.15)
    ax_norm.set_title("after nnU-Net normalisation, whole scan", fontsize=13, loc="left")
...
def main():
    """Generate the before/after nnU-Net normalisation figure."""
    ...
    title_block(
        fig,
        "Scan Intensity: Before and After nnU-Net Normalisation",
    ...
    out_path = output_path(args.profile_dir, "nnunet_normalisation.png")
