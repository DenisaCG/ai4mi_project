...
of voxels so rare but non-zero ranges stay visible. A grey band repeated
...
nnU-Net's CT-normalisation clip range, recomputed from its own foreground
sample.
...
def set_mirrored_log_ticks(ax, values: tuple[float, ...] = (0.001, 1, 30), floor: float = FLOOR_PCT) -> None:
    """Set y ticks labelled with percentages, mirrored above and below zero.
...
def main():
    """Draw the whole-scan-vs-labelled-voxels small multiples from the intensity histograms."""
    ...
    title_block(
        fig,
        "Scan Intensity: Whole Scan vs Labelled Voxels, per Patient",
