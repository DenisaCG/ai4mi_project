#!/usr/bin/env python3
"""
3D view of eight axial slices through one patient's CT scan, coloured by
intensity.
...
pixel is coloured by its HU value on the "mako" colormap, normalised over
...
def slice_colors(volume: np.ndarray, indices: np.ndarray, cmap, norm: Normalize) -> list[np.ndarray]:
    """Colour the chosen axial slices by HU, with see-through low values.

    Args:
        volume: HU volume (x, y, z).
        indices: Slice indices to colour.
