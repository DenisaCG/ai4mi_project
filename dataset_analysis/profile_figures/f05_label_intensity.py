#!/usr/bin/env python3
"""
Label intensity: HU histograms of the image values inside labels 1, 2 and 3,
one ridge row per patient plus a row pooling all patients, with nnU-Net's
foreground intensity sample and its clip values overlaid.
...
def binned_counts(counts: np.ndarray, start: int, lo: int, hi: int, width: int) -> tuple[np.ndarray, np.ndarray]:
    ...
    Returns:
        Bin centres and voxel counts per bin.
    """
...
def main():
    ...
    bins = {key: binned_counts(*hists[key], *HU_RANGE, BIN_HU) for key in hists if key[1] in GROUPS + [SAMPLE]}
