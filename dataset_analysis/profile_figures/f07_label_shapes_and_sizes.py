...
Usage:
    python dataset_analysis/profile_figures/f07_label_shapes_and_sizes.py --profile-dir figures/profile \
...
def label_boxes(labels: pd.DataFrame) -> pd.DataFrame:
    """Box around labels 1-3 together, per patient, in mm.
    ...
    """
    present = labels[labels.label.isin(LABELS) & (labels.voxels > 0)]
    agg = {f"{a}_min_mm": "min" for a in "xyz"} | {f"{a}_max_mm": "max" for a in "xyz"}
    return present.groupby("patient").agg(agg)


def draw_shapes(labels: pd.DataFrame, data_dir: Path, out_dir: Path) -> None:
    ...
    for i, (patient, patient_meshes) in enumerate(zip(patients, meshes)):
        ...
        for a, setter in zip("xyz", (ax.set_xlim, ax.set_ylim, ax.set_zlim)):
            centre = (boxes.at[patient, f"{a}_min_mm"] + boxes.at[patient, f"{a}_max_mm"]) / 2
            setter(centre - half[a], centre + half[a])
    ...
    handles = [plt.Rectangle((0, 0), 1, 1, color=LABEL_COLORS[v], label=f"label {v}") for v in LABELS]
    ...
    header(
        ...
        "Labels 1–3 of every patient at the same physical scale and camera angle.",
        subtitle_y=0.955,
    )
...
def draw_sizes(labels: pd.DataFrame, out_dir: Path) -> None:
    ...
    """
    table = labels[labels.label.isin(LABELS)].assign(voxels_thousands=lambda d: d.voxels / 1000)
    ...
    palette = {v: LABEL_COLORS[v] for v in LABELS}
    ...
    handles = [
        plt.Line2D([], [], marker="o", ls="", color=LABEL_COLORS[v], markersize=9, label=f"label {v}") for v in LABELS
    ]
...
def main():
    ...
    apply_ticks_style()
    draw_sizes(labels, out_dir)
    draw_shapes(labels, args.data_dir, out_dir)
