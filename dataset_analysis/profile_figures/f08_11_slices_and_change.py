#!/usr/bin/env python3
"""
Slices per label and slice-to-slice area change for labels 1, 2 and 3.
...
contain each label:
  ...
  slices_and_change_joint.png  one 2D histogram per label of every neighbouring
...
def _slices_histogram(ax, labels: pd.DataFrame) -> None:
    sns.histplot(
        labels,
        x="slices_with_label",
        hue="name",
        hue_order=[f"label {k}" for k in LABELS[::-1]],
        palette=PALETTE,
        multiple="stack",
        binwidth=4,
        edgecolor=".3",
...
def _bands(ax, df: pd.DataFrame, column: str, ylabel: str, title: str) -> None:
...
def _fig_bands(df: pd.DataFrame, labels: pd.DataFrame, out_dir: Path) -> None:
    fig = plt.figure(figsize=(18, 12))
    gs = fig.add_gridspec(2, 2, width_ratios=[1, 2.2], hspace=0.35, wspace=0.18)
    _slices_histogram(fig.add_subplot(gs[0, 0]), labels)
    ax = fig.add_subplot(gs[0, 1])
    _bands(ax, df, "area_pct_max", "area on the slice (% of that label's largest slice)", "Label area along its slices")
    ax.legend(
        handles=[plt.Line2D([], [], color=LABEL_COLORS[k], lw=2.4, label=f"label {k}") for k in LABELS]
        + [
    ...
    _bands(ax, df, "change_pct", CHANGE_LABEL, "Slice-to-slice area change along the label")
...
def _fig_joint(df: pd.DataFrame, labels: pd.DataFrame, out_dir: Path) -> None:
    fig = plt.figure(figsize=(19, 11))
    outer = fig.add_gridspec(2, 3, height_ratios=[1, 1.6], hspace=0.35, wspace=0.25)
    _slices_histogram(fig.add_subplot(outer[0, :]), labels)
...
def main():
    ...
    slices = slices[slices["label"].isin(LABELS)]
    labels = labels[labels["label"].isin(LABELS)].assign(name=lambda d: "label " + d["label"].astype(str))

    apply_ticks_style()
    out_dir = out_subdir(args.profile_dir, "08-11_slices_and_change")
    df = slice_changes(slices)
    _fig_bands(df, labels, out_dir)
    _fig_joint(df, labels, out_dir)
