...
as opaque outlined bars (one colour per organ) in thousand voxels per 10 HU
...
MIN_TISSUE_HIGH = 300
NNUNET_FOREGROUND_SAMPLES = 10e7  # labelled voxels nnU-Net samples over all cases for its statistics
BIN_HU = 10
WIDTH_IN = 12
...
FIXED_TICKS = (-1000, -900, -600)  # labelled ticks below the soft-tissue window
NNUNET_COLOR = "#111111"  # nnU-Net's statistics of the labelled voxels
...
def tick_positions(high: int) -> tuple[list[int], list[int]]:
    """HU values of the labelled (major) and unlabelled (minor) ticks of the x axis.
...
def bar_style(key, color: str) -> dict:
    """Fill, outline colour and outline width of a label's bars.

    Args:
        key: Label number.
        color: The label's colour.
...
def foreground_stats(per_patient: list[tuple[np.ndarray, int]]) -> dict:
    """Statistics as nnU-Net's fingerprint takes them from the labelled voxels.

    nnU-Net draws the same number of labelled voxels from every case, with replacement, so
    a small case counts as much as a large one. This uses the expected result of that draw.

    Args:
        per_patient: (counts, start) 1 HU histogram of all labelled voxels of each patient.
...
def load_dataset(profile_dir: Path) -> dict:
    ...
    Returns:
        ...
        stats per label and foreground (all labelled voxels together).
    """
...
def bin_values(data: dict, key: int, high: int) -> tuple[np.ndarray, np.ndarray]:
    ...
    Returns:
        Bin centres and the label's thousand voxels in each bin.
    """
...
def main():
    ...
    caption = [
        ...
        f"bars = labelled voxels per {BIN_HU} HU bin.",
    ]
