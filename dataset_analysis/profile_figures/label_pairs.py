...
# One earth-palette colour per label pair (same family as the scan field-of-view figure); the
# direction is the shade: full colour between slices, a light tint of it within a slice.
...
def contact_colors(pairs: list[tuple[int, int]]) -> dict[tuple[tuple[int, int], str], str]:
    """Colour for every (pair, direction).

    Args:
        pairs: Label pairs that actually occur in this dataset.

    Returns:
        {(pair, direction): hex colour}: the pair's colour between slices, a tint of it within a slice.
    """
