...
def slice_rows(mask: np.ndarray, zooms: np.ndarray) -> list[dict]:
    ...
    Returns:
        One dict per slice with index, height, area, left-right and front-back size, and piece count.
    """
    rows = []
...
def identical_neighbour_slices(ct: np.ndarray) -> int:
...
def profile_patient(patient_dir: Path) -> dict:
    ...
    patient["identical_neighbour_slices"] = identical_neighbour_slices(ct)
