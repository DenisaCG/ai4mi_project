...
def label_boxes(labels: pd.DataFrame, label: int, reference: int | None = None) -> tuple[np.ndarray, np.ndarray]:
    ...
    Args:
        ...
        reference: If given, shift each patient's box so that patient's centre of this label is at 0.
...
def size_figure(labels: pd.DataFrame, out: Path) -> None:
    """One 3D panel per label, all boxes around one centre."""
...
def position_figure(labels: pd.DataFrame, out: Path) -> None:
    """All labels in one 3D space, placed around each patient's label 2 centre."""
    ...
    handles = line_handles(
        [(f"label {label}", {"color": COLORS[label], "lw": 6}) for label in LABELS]
        + [
