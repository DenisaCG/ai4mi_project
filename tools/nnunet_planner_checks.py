...
# No 0 bin: voxel centres are never closer than half a voxel to the surface.
...
def boundary_hu_profile(ct: np.ndarray, mask: np.ndarray, zooms: np.ndarray) -> list[float] | None:
    ...
    spacing does not mix voxels from neighbouring slices into the 1 mm bins.
...
def load_cache(out_dir: Path) -> dict | None:
    ...
    Returns:
        The compute() dict, or None if the cache is missing or from an older
